...
(function() {
    ...
    let isPaused = false; // NEW: Pause auto-logout when user modal is open
    ...
    /**
     * Close SweetAlert if visible (ONLY auto-logout warnings, NOT user-initiated modals)
     */
    function closeSwalIfVisible() {
        if (window.Swal && Swal.isVisible()) {
            // HANYA tutup modal auto-logout warning, JANGAN tutup user-initiated modals
            // Cek apakah ini adalah warning dialog inaktivitas
            const swalPopup = document.querySelector('.swal2-popup');
            if (swalPopup) {
                const titleElement = swalPopup.querySelector('.swal2-title');
                if (titleElement) {
                    const title = titleElement.textContent.toLowerCase();
                    // Hanya tutup jika ini adalah auto-logout warning
                    if (title.includes('peringatan inaktivitas') ||
                        title.includes('sesi berakhir') ||
                        title.includes('sesi dilanjutkan')) {
                        Swal.close();
                    }
                }
            }
    ...
    function resetInactivityTimer() {
        // Don't reset if not initialized, logout is in progress, or paused
        if (!isInitialized || isLogoutInProgress || isPaused) {
            return;
        }

        // JANGAN reset timer jika ada SweetAlert modal yang sedang terbuka
        // Ini mencegah popup konfirmasi logout/user action tertutup
        if (window.Swal && Swal.isVisible()) {
            const swalPopup = document.querySelector('.swal2-popup');
            if (swalPopup) {
                const titleElement = swalPopup.querySelector('.swal2-title');
                if (titleElement) {
                    const title = titleElement.textContent.toLowerCase();
                    // Jika ini bukan auto-logout warning, JANGAN reset timer
                    if (!title.includes('peringatan inaktivitas') &&
                        !title.includes('sesi berakhir') &&
                        !title.includes('sesi dilanjutkan')) {
                        return; // User sedang berinteraksi dengan modal, jangan ganggu
                    }
                }
            }
        }
        ...
        // Close any open SweetAlert (hanya auto-logout warnings)
        closeSwalIfVisible();
    ...
    function initAutoLogout() {
        ...
        // Expose functions for external use (e.g., sidebar logout button)
        window.pauseAutoLogoutReset = function() {
            if (isInitialized) {
                isPaused = true;
                console.log('Auto-logout timer paused');
            }
        };
        ...
        window.resumeAutoLogoutReset = function() {
            if (isInitialized) {
                isPaused = false;
                resetInactivityTimer();
                console.log('Auto-logout timer resumed');
            }
        };

        // Check if SweetAlert modal is open (user-initiated, not auto-logout)
        function isUserModalOpen() {
            ...
            if (swalPopup) {
                ...
                if (titleElement) {
                    const title = titleElement.textContent.toLowerCase();
                    // Return true if this is a user modal (not auto-logout warning)
                    return !title.includes('peringatan inaktivitas') &&
                           !title.includes('sesi berakhir') &&
                           !title.includes('sesi dilanjutkan');
                }
            }
