    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white p-6 rounded-xl shadow border">
                ...
                <form id="formUpdateStatus" action="{{ route('admin.status.aktelahir', $berkas->uuid) }}" method="POST" class="hidden">
                    @csrf
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.AdminVerificationConfirm) {
        window.AdminVerificationConfirm.bindAcceptButton({
            ...
            loadingHtml: '<div class="flex flex-col items-center gap-3 py-2"><i class="fas fa-circle-notch fa-spin text-4xl text-green-500"></i><p class="text-gray-600 text-sm">Sedang memproses data akta lahir...</p></div>'
        });
    }

    if (btnTolak && window.AdminVerificationConfirm) {
        window.AdminVerificationConfirm.bindRejectButton({
            loadingHtml: '<div class="flex flex-col items-center gap-3 py-2"><i class="fas fa-circle-notch fa-spin text-4xl text-red-500"></i><p class="text-gray-600 text-sm">Sedang memproses penolakan akta kelahiran...</p></div>'
        });
    }
});
</script>
