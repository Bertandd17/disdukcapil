...
function openUploadModal(uuid, nama) {
    ...
    form.action = @json(route('admin.lahir-mati.upload-berkas', ['uuid' => '__UUID__'])).replace('__UUID__', uuid);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.AdminVerificationConfirm) {
        window.AdminVerificationConfirm.bindUploadForm({
            docLabel: 'Surat Keterangan Lahir Mati',
            confirmTitle: 'Konfirmasi Upload Berkas',
            loadingTitle: 'Mengunggah Berkas',
            loadingHtml: '<div class="flex flex-col items-center gap-3 py-2"><i class="fas fa-circle-notch fa-spin text-4xl text-green-500"></i><p class="text-gray-600 text-sm">Sedang mengunggah berkas lahir mati...</p></div>'
        });
    }
    @if(session('success'))
