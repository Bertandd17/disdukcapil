...
<div class="container-fluid p-6 bg-gray-50 min-h-screen">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-800">Kelola Penerbitan Akte Kematian</h1>
    </div>
...
function openUploadModal(uuid, nama) {
    ...
    form.action = @json(route('admin.akte-kematian.upload-berkas', ['uuid' => '__UUID__'])).replace('__UUID__', uuid);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.AdminVerificationConfirm) {
        window.AdminVerificationConfirm.bindUploadForm({
            docLabel: 'Akta Kematian',
            confirmTitle: 'Konfirmasi Upload Berkas',
            loadingTitle: 'Mengunggah Berkas',
            loadingHtml: '<div class="flex flex-col items-center gap-3 py-2"><i class="fas fa-circle-notch fa-spin text-4xl text-green-500"></i><p class="text-gray-600 text-sm">Sedang mengunggah berkas akta kematian...</p></div>'
        });
    }
    @if(session('success'))
