...
async function loadCalendarData() {
    ...
    try {
        const response = await fetch(`{{ route('api.pernikahan.admin.calendar') }}?year=${year}&month=${month}`);
...
async function showDetail(id) {
    ...
    try {
        const response = await fetch(`{{ route('api.pernikahan.admin.detail', ':id') }}`.replace(':id', id));
...
async function executeConfirm() {
    ...
    try {
        ...
        if (action === 'approve') {
            url = '{{ route('api.pernikahan.admin.approve', ':id') }}'.replace(':id', id);
            method = 'POST';
            body = JSON.stringify({});
        } else if (action === 'reject') {
            url = '{{ route('api.pernikahan.admin.reject', ':id') }}'.replace(':id', id);
            method = 'POST';
            body = JSON.stringify({ alasan: reason });
        } else if (action === 'reject_doc') {
            url = '{{ route('api.pernikahan.admin.reject-doc', ':id') }}'.replace(':id', id);
            method = 'POST';
            body = JSON.stringify({ alasan: reason });
        }
...
async function verifyAllDocuments(id) {
    try {
        const response = await fetch('{{ route('api.pernikahan.admin.verify-all', ':id') }}'.replace(':id', id), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
async function handleFileUpload(input, id, type) {
    ...
    try {
        ...
        const response = await fetch('{{ route('api.pernikahan.admin.upload-berkas', ':id') }}'.replace(':id', id), {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
