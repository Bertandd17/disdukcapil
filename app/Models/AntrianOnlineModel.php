...
/**
 * AntrianOnlineModel - Model untuk tabel antrian_online
 * 
...
class AntrianOnlineModel extends Model
{
    ...
    {
        return $this->hasMany(LacakBerkasModel::class, 'antrian_online_id', 'antrian_online_id');
    }
    ...
    {
        $layananTujuan = LayananModel::findByIdentifier($layananId);
        ...
        if ($this->layanan_id !== $layananTujuan->layanan_id) {
            $layananAsal = $this->relationLoaded('layanan')
                ? $this->layanan
                : LayananModel::findByIdentifier($this->layanan_id);
