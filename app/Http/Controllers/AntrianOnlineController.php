...
class AntrianOnlineController extends Controller
{
    ...
    public function Test_Search()
    {
        try {
            // Test basic query
            $count = AntrianOnlineModel::count();
            
            // Test with relationships
            $samples = AntrianOnlineModel::with(['layanan', 'lacak_berkas'])
                ->limit(3)
                ->get();
    ...
    public function Tambah_Antrian(Request $request)
    {
        try {
            ...
            $existingAntrian = AntrianOnlineModel::where('nik', $request->nik)
            ...
            if ($existingAntrian) {
                $layanan = LayananModel::find($request->layanan_id);
            ...
            return \Illuminate\Support\Facades\DB::transaction(function () use ($request) {
                ...
                while (AntrianOnlineModel::where('nomor_antrian', $nomor_antrian)->exists()) {
                    $nomor_antrian = $this->Generate_Nomor_Antrian();
                    $attempts++;
                ...
                // Buat antrian baru
                $antrian = AntrianOnlineModel::create($data_antrian);
    ...
    public function Cari_Antrian(Request $request)
    {
        ...
        $query = AntrianOnlineModel::query();
    ...
    {
        $antrian = AntrianOnlineModel::with(['layanan', 'lacak_berkas'])
    ...
    public function Get_Statistik_Antrian()
    {
        ...
        $total_antrian = AntrianOnlineModel::whereDate('created_at', $hari_ini)->count();

        // Antrian menunggu
        $antrian_menunggu = AntrianOnlineModel::whereDate('created_at', $hari_ini)
        ...
        $antrian_diproses = AntrianOnlineModel::whereDate('created_at', $hari_ini)
            ->whereIn('status_antrian', $status_diproses)
            ->count();

        // Antrian selesai (Siap Pengambilan)
        $antrian_selesai = AntrianOnlineModel::whereDate('created_at', $hari_ini)
    ...
    public function Lacak_Berkas(Request $request)
    {
        ...
        $query = AntrianOnlineModel::with(['layanan', 'lacak_berkas' => function ($q) {
    ...
    public function Finalisasi_Antrian(Request $request, string $antrianId): JsonResponse
    {
        ...
        if (strlen($nik) === 16) {
            ...
            $duplicate = AntrianOnlineModel::where('nik', $nik)
                ...
                ->first();
            if ($duplicate) {
                $layanan = LayananModel::find($draft->layanan_id);
    ...
    private function Hapus_Draft_Sementara(?AntrianOnline $antrian): void
    {
        ...
        try {
            DB::transaction(function () use ($antrian): void {
                LacakBerkasModel::where('antrian_online_id', $antrian->antrian_online_id)->delete();
    ...
    {
        $antrian = AntrianOnlineModel::where('nomor_antrian', $nomor_antrian)->first();
    ...
    public function Cari_Antrian_Post(Request $request)
    {
        ...
        $query = AntrianOnlineModel::with(['layanan'])
        ...
        try {
            ...
            if (!$antrian) {
                Log::warning('Antrian not found', [
                    ...
                    'all_antrians' => AntrianOnlineModel::select('nomor_antrian')->limit(5)->get()->pluck('nomor_antrian')->toArray(),
                ]);
                return response()->json([
                    'success' => false,
    ...
    public function checkDailyLimit(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $layanan = LayananModel::findByIdentifier($layananId);
            ...
            $result = AntrianOnlineModel::checkDailyLimit($nik, $layanan->layanan_id);
