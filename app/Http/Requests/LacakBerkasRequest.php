...
/**
 * Form Request untuk validasi Lacak Berkas
 *
 * Security Features:
...
class LacakBerkasRequest extends FormRequest
{
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function ($validator) {
            // Minimal salah satu harus diisi
            if (empty($this->nomor_antrian) && empty($this->nama_lengkap)) {
                $validator->errors()->add('nomor_antrian', 'Masukkan nomor antrian atau nama lengkap');
                $validator->errors()->add('nama_lengkap', 'Masukkan nomor antrian atau nama lengkap');
            }
        });
    }
